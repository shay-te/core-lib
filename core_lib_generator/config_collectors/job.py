from core_lib.helpers.shell_utils import prompt_str, prompt_timeframe, prompt_yes_no
...
def generate_job_template(core_lib_name: str) -> list:
    ...
    while add_job:
        class_name = any_to_pascal(prompt_str('Please enter the Class Name for the job (CamelCase)', None, False, is_exists_jobs))
        snake_class_name = camel_to_snake(class_name)
        initial_delay = prompt_timeframe(
            'Please set the initial delay for the job (boot, startup, 1s, 1m, 1h, 1h30m ...)', 'startup'
        )
        frequency = prompt_timeframe('Please set the frequency of the job (1s, 1m, 1h, 1h30m ...)', '', True)
        add_job = prompt_yes_no('Do you want to add another job?', False)
