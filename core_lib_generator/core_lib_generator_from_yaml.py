...
from core_lib_generator.file_generators.cache_generator import CacheGenerateTemplate
...
class CoreLibGenerator:
    ...
    def _generate_template(self, file_path: str, yaml_data: dict, template_generate):
        ...
        with open(generator.get_template_file(yaml_data), 'r') as template_file:
            new_file = generator.handle(template_file.read(), yaml_data, self.snake_core_lib_name)
    ...
    def generate_cache(self):
        if 'cache' in self.core_lib_config:
            self._generate_template(
                f'{self.snake_core_lib_name}/core_lib/{self.snake_core_lib_name}.py',
                self.core_lib_config['cache'],
                CacheGenerateTemplate,
            )
