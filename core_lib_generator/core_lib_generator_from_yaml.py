...
for name in db_entities_list:
    new_file_name = f'{camel_to_snake(core_lib_name)}/{camel_to_snake(core_lib_name)}/data_layers/data/db/{name.lower()}.py'
    shutil.copy(
    ...
    add_columns_to_entity(
        new_file_name,
        core_lib_entities[name]['columns'],
    )
