...
class EntityGenerateTemplate(TemplateGenerate):
    def generate(self, template_file: str, yaml_data: dict, core_lib_name: str, file_name: str) -> str:
        updated_file = _add_columns_to_entity(template_file, yaml_data['columns'])
...
def _add_columns_to_entity(template_file: str, columns: dict) -> str:
    ...
    updated_file = template_file.replace('# template_column', '\n'.join(columns_str))
