...
class SetupGenerateTemplate(TemplateGenerator):
    def generate(self, template_content: str, yaml_data: dict, core_lib_name: str, file_name: str) -> str:
        ...
        updated_file = updated_file.replace('template_full_name', yaml_data['author'])
        updated_file = updated_file.replace('template_email', yaml_data['author_email'])
        updated_file = updated_file.replace('template_description', yaml_data['description'])
        updated_file = updated_file.replace('template_url', yaml_data['url'])
