...
SQLAlchemy session management — committing, flushing, and closing — has to happen on every request, and forgetting any step causes silent bugs or connection leaks. `SqlAlchemyConnectionFactory` handles the full session lifecycle automatically inside `with conn.get() as session:`, so you write queries without managing transactions manually.

> **Where it fits:** Connection layer for relational databases. Create it in `CoreLib.__init__`, pass it to SQLAlchemy `DataAccess` classes, and keep raw sessions out of `Service` code.
...
*core_lib.connection.sql_alchemy_connection_factory.SqlAlchemyConnectionFactory* [[source]](https://github.com/shay-te/core-lib/blob/master/core_lib/connection/sql_alchemy_connection_factory.py#L9){:target="_blank"}
