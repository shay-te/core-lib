...
Neo4j sessions should be opened for a unit of work and closed after the query finishes. `Neo4jConnectionFactory` keeps the driver and exposes each session through Core-Lib's normal `with conn.get() as session:` pattern. The session is closed when the `with` block exits.

> **Where it fits:** Connection layer for graph data. Create it in `CoreLib.__init__`, pass it to graph `DataAccess` classes, and keep Cypher queries out of `Service` classes.
...
- **`config`** *`(DictConfig)`*: Neo4j section from your Core-Lib config.
