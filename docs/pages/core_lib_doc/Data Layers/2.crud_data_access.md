...
Writing `get`, `create`, `update`, and `delete` against every SQLAlchemy model is tedious and identical every time. `Core-Lib` provides `CRUD` base classes that implement all four operations so your `DataAccess` only needs to define what makes it different.
...
class CustomerCRUDDataAccess(CRUDDataAccess):
    ...
    def __init__(self, db: SqlAlchemyConnectionFactory):
        CRUDDataAccess.__init__(self, Customer, db, CustomerCRUDDataAccess.rules_validator)

# db is a SqlAlchemyConnectionFactory instance from your CoreLib
customer = CustomerCRUDDataAccess(db)
...
class CustomerCRUDSoftDeleteWithTokenDataAccess(CRUDSoftDeleteWithTokenDataAccess):
    def __init__(self, db: SqlAlchemyConnectionFactory):
        CRUDSoftDeleteWithTokenDataAccess.__init__(self, Customer, db)
...
class CRUDDataAccess(DataAccess, CRUD):
    ...
    def __init__(self, db_entity, db: SqlAlchemyConnectionFactory, rule_validator: RuleValidator):
...
class CustomerCRUDDataAccess(CRUDDataAccess):
    def __init__(self, db: SqlAlchemyConnectionFactory):
        CRUDDataAccess.__init__(self, Customer, db)
...
class CustomerCRUDSoftDeleteDataAccess(CRUDSoftDeleteDataAccess):
    ...
    def __init__(self, db: SqlAlchemyConnectionFactory):
        CRUDSoftDeleteDataAccess.__init__(self, Customer, db)
