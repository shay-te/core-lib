...
class TestUpdateValidate(unittest.TestCase):
    ...
    def test_1_expressions(self):
        ...
        allowed_update_types = [
            ...
            ValueRuleValidator('age_from', int, nullable=False,
                               custom_validator=lambda value: 0 <= value > USER_MIN_AGE),
        ...
        self.assertRaises(PermissionError, rules_validator.validate_dict, {'gender': 1, 'prohibited_key': 'value'},
                          prohibited_keys='prohibited_key')  # prohibited_keys locally on the function
        ...
        self.assertDictEqual(rules_validator.validate_dict(json_data, strict_mode=False,
                                                           prohibited_keys=["now_key_to_reset_the_forhibeded_email"]),
                             json_data)

        new_dict = rules_validator.validate_dict(json_data, strict_mode=False, strict_output=True,
                                                 prohibited_keys=["now_key_to_reset_the_forhibeded_email"])
